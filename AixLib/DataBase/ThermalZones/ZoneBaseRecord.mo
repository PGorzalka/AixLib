...
record ZoneBaseRecord "Base record definition for zone records"
  ...
  parameter Real hHeat "Upper limit controller output" annotation(Dialog(tab="Energy Supply", group="Heating", enable=HeaterOn==true));
  parameter Real lHeat "Lower limit controller output" annotation(Dialog(tab="Energy Supply", group="Heating", enable=HeaterOn==true));
  parameter Real KRHeat "Gain of the controller" annotation(Dialog(tab="Energy Supply", group="Heating", enable=HeaterOn==true));
  parameter Modelica.Units.SI.Time TNHeat "Time constant of the controller" annotation(Dialog(tab="Energy Supply", group="Heating", enable=HeaterOn==true));
  ...
  parameter Boolean withIdealThresholds=false
    "Sets if the threshold temperatures for ideal heater and cooler should
        be used" annotation(Dialog(tab="Energy Supply", group="Heating and Cooling"));

  // AHU
  parameter Boolean withAHU=true
    "Zone is connected to central air handling unit" annotation(Dialog(tab="Energy Supply", group="Air Handling Unit"));
  parameter Real minAHU(unit="m3/(h.m2)")
    "Minimum specific air flow supplied by the AHU" annotation(Dialog(tab="Energy Supply", group="Air Handling Unit", enable=withAHU));
  parameter Real maxAHU(unit="m3/(h.m2)")
    "Maximum specific air flow supplied by the AHU" annotation(Dialog(tab="Energy Supply", group="Air Handling Unit", enable=withAHU));
