...
block LimPID
  ...
  parameter AixLib.Types.Reset reset = AixLib.Types.Reset.Disabled
    "Type of controller output reset"
    annotation(Evaluate=true, Dialog(group="Integrator reset"));

  parameter Real y_reset=xi_start
    "Value to which the controller output is reset if the boolean trigger has a rising edge, used if reset == AixLib.Types.Reset.Parameter"
...
equation
  ...
  if reset <> AixLib.Types.Reset.Input then
