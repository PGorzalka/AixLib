...
equation
  ...
  if datSou == AixLib.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HDifHor then
  ...
  if datSou == AixLib.BoundaryConditions.Types.RadiationDataSource.Input_HDirNor_HGloHor then
  ...
  if datSou == AixLib.BoundaryConditions.Types.RadiationDataSource.Input_HGloHor_HDifHor then
      HDirNor = max(0, min(
        solCon,
        (HGloHor_in_internal -HDifHor_in_internal)*
          AixLib.Utilities.Math.Functions.spliceFunction(
            x=cos(zen),
            pos=AixLib.Utilities.Math.Functions.inverseXRegularized(cos(zen), epsCos),
            neg=0,
            deltax=epsCos)))
