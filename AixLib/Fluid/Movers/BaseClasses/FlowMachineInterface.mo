...
protected
  ...
  final parameter Boolean prePre=
    preVar == AixLib.Fluid.Movers.BaseClasses.Types.PrescribedVariable.PressureDifference or
    preVar == AixLib.Fluid.Movers.BaseClasses.Types.PrescribedVariable.FlowRate
  ...
  final parameter
    AixLib.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur1(
    final n = nOri,
  ...
  parameter
    AixLib.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur2(
    final n = nOri + 1,
    ...
    "Volume flow rate vs. total pressure rise with correction for pump resistance added";
  parameter
    AixLib.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pCur3(
    final n = nOri + 2,
    V_flow = if (haveVMax and haveDPMax) or (nOri == 2) then
               zeros(nOri + 2)
  ...
  parameter Real powDer[size(per.power.V_flow,1)]=
   if per.use_powerCharacteristic then
     AixLib.Utilities.Math.Functions.splineDerivatives(
                   x=per.power.V_flow,
                   y=per.power.P,
                   ensureMonotonicity=AixLib.Utilities.Math.Functions.isMonotonic(x=per.power.P,
                                                                                     strict=false))
   else
...
function getPerformanceDataAsString
  input AixLib.Fluid.Movers.BaseClasses.Characteristics.flowParameters pressure
      "Performance data";
...
the simulation stops.");
  ...
  if (haveVMax and haveDPMax) or (nOri == 2) then  // ----- Curve 1
    ...
    preDer1= AixLib.Utilities.Math.Functions.splineDerivatives(x=pCur1.V_flow,
      y=pCur1.dp);
  ...
  elseif haveVMax or haveDPMax then  // ----- Curve 2
    ...
    preDer1= zeros(nOri);
    preDer2= AixLib.Utilities.Math.Functions.splineDerivatives(x=pCur2.V_flow,
      y=pCur2.dp);
    preDer3= zeros(nOri + 2);
  else  // ----- Curve 3
    ...
    preDer1= zeros(nOri);
    preDer2= zeros(nOri + 1);
    preDer3= AixLib.Utilities.Math.Functions.splineDerivatives(x=pCur3.V_flow,
      y=pCur3.dp);
  end if;
 ...
 motDer = if per.use_powerCharacteristic then zeros(size(per.motorEfficiency.V_flow,
    ...
    ensureMonotonicity=AixLib.Utilities.Math.Functions.isMonotonic(x=per.motorEfficiency.eta,
      strict=false));
...
equation
  ...
  if (computePowerUsingSimilarityLaws == false) and preVar <> AixLib.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed then
    r_N=1;
  else
  ...
  if per.use_powerCharacteristic then
    ...
    // a division by zero.
    eta = WFlo / AixLib.Utilities.Math.Functions.smoothMax(x1=PEle, x2=1E-5, deltaX=1E-6);
    ...
    // a division by zero.
    PEle = WFlo / AixLib.Utilities.Math.Functions.smoothMax(x1=eta, x2=1E-5, deltaX=1E-6);
    eta = etaHyd * etaMot;
