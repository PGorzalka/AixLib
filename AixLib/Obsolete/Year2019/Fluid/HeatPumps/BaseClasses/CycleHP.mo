...
public
  ...
  parameter Modelica.Units.SI.Temperature T_conMax=338.15
    ...
    annotation (Dialog(group="Heat Pump cycle"));
  ...
  parameter Modelica.Units.SI.Power P_eleAdd=0
  ...
  parameter Modelica.Units.SI.MassFlowRate mFlow_conNominal=0.5
    "Nominal mass flow rate in condenser" annotation (Dialog(
  ...
  parameter Modelica.Units.SI.MassFlowRate mFlow_evaNominal=0.5
    "Nominal mass flow rate in evaporator" annotation (Dialog(
      group="Mass flow correction",
      tab="Advanced",
      enable=(CorrFlowEv and capCalcType == 1)));

  Modelica.Units.SI.Power P_ele;
  Modelica.Units.SI.Power P_eleChar;
  Modelica.Units.SI.HeatFlowRate Qdot_eva;
  Modelica.Units.SI.HeatFlowRate Qdot_con;
  Modelica.Units.SI.HeatFlowRate Qdot_conChar;
  Real CoP;
  Real CoP_corr;
  Real CoP_char;
  Real N;
...
protected
  ...
  Modelica.Blocks.Interfaces.RealInput P_eleTableInternal
    "Needed to connect to conditional model";
  Modelica.Units.SI.MassFlowRate mFlow_conNom;
  Modelica.Units.SI.MassFlowRate mFlow_evaNom;
...
public
  ...
  Modelica.Blocks.Tables.CombiTable2Ds Qdot_ConTable(
    ...
    smoothness=smoothness) if
                            not (capCalcType == 1) annotation (extent=[-60,40;
...
<p>
  Heat pump refrigerant circuit black box model, no controllers
  included. It is used within the HeatPump model and delivers the basic
  functionality of the heat pump. The HeatPump model only adds the
  external heat exchangers and according connectors.
</p>
<h4>
...
<p>
  ...
  heat pumps. Therefore two possibilities exist, to add dynamic
  behavior:
</p>
<ol>
