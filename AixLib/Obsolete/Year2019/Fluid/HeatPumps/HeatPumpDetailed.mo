...
model HeatPumpDetailed
  ...
  parameter Modelica.Units.SI.PressureDifference dp_conNominal=0
  ...
  parameter Modelica.Units.SI.PressureDifference dp_evaNominal=0
    "Pressure loss at nominal mass flow in evaporator"
    annotation (Dialog(tab="Evaporator, Condenser", group="Evaporator"));

  parameter Modelica.Units.SI.Volume volume_eva=0.004
    "External medium volume in heat exchanger"
    annotation (Dialog(tab="Evaporator, Condenser", group="Evaporator"));
  parameter Modelica.Units.SI.Volume volume_con=0.004
    "External medium volume in heat exchanger"
    annotation (Dialog(tab="Evaporator, Condenser", group="Condenser"));
  ...
  parameter Modelica.Units.SI.Temperature T_startEva=273.15
    "Initial evaporator temperature" annotation (Evaluate=true, Dialog(
      tab="Evaporator, Condenser",
      group="Initialization",
      enable=initEvaporatorVol));
  parameter Modelica.Units.SI.Temperature T_startCon=308.15
    "Initial condenser temperature" annotation (Evaluate=true, Dialog(
      tab="Evaporator, Condenser",
      group="Initialization",
  ...
  parameter Boolean PT1_cycle=false "First Order model for capacity"
    annotation (Dialog(group="Start/stop behavior", tab="Advanced"), choices(
        checkBox=true));
  parameter Modelica.Units.SI.Time timeConstantCycle=1
    "Time constant for first order model" annotation (Dialog(
  ...
  parameter Real factorScale=1
    ...
    annotation (Dialog(group="Assumptions", tab="Advanced"));
  parameter Modelica.Units.SI.Power P_eleAdd=0 "Additional electric power when heat pump is on (not influenced through 
    scaling factor)" annotation (Dialog(group="Assumptions", tab="Advanced"));
  ...
  parameter Boolean CorrFlowCo=false
    ...
    annotation (Dialog(group="Mass flow correction", tab="Advanced"), choices(
        checkBox=true));
  parameter Modelica.Units.SI.MassFlowRate mFlow_conNominal=0.5 "Nominal mass flow rate in condenser (only with polynomials, as already 
    given in data tables)" annotation (Dialog(
  ...
  parameter Boolean CorrFlowEv=false
    ...
    annotation (Dialog(group="Mass flow correction", tab="Advanced"), choices(
        checkBox=true));
  parameter Modelica.Units.SI.MassFlowRate mFlow_evaNominal=0.5 "Nominal mass flow rate in evaporator (only for polynomials, as already 
    given in data tables)" annotation (Dialog(
      group="Mass flow correction",
      tab="Advanced",
...
public
  ...
  parameter Modelica.Units.SI.ThermalConductance R_loss=1
...
public
  ...
  parameter Modelica.Units.SI.Temperature T_conMax=338.15
...
<ol>
  <li>Icing and Defrosting: Simple model that uses the evaporator inlet
  ...
  <li>Allowed sink temperature: A maximum condenser outlet temperature
...
<ul>
  ...
  <li>Defrost: No direct implementation of defrost behavior. This heat
