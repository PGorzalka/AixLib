...
extends AixLib.Obsolete.BaseClasses.ObsoleteModel;
  ...
  parameter Real exponent=(if selectable then radiatorType.Exponent else 1.29)
    annotation (Dialog(tab="Geometry and Material", group="Geometry", enable=not selectable));
  parameter Modelica.Units.SI.Length length=(if selectable then radiatorType.length
       else 1) "Length of radiator, in m" annotation (Dialog(
      tab="Geometry and Material",
      group="Geometry",
      enable=not selectable));
  parameter Modelica.Units.SI.Length height=(if selectable then radiatorType.height
       else 0.6) "Height of raditor, in m" annotation (Dialog(
      tab="Geometry and Material",
      group="Geometry",
      enable=not selectable));
  ...
  parameter Modelica.Units.SI.LinearDensity massSteel=(if selectable then
    ...
    "Material mass of radiator per m, in kg/m" annotation (Dialog(
      ...
      group="Geometry",
      enable=not selectable));
  parameter Modelica.Units.SI.Density densitySteel=(if selectable then
      radiatorType.DensitySteel else 7900)
    "Specific density of steel, in kg/m3" annotation (Dialog(
      tab="Geometry and Material",
      group="Material",
      enable=not selectable));

  parameter Modelica.Units.SI.SpecificHeatCapacity capacitySteel=(if selectable
       then radiatorType.CapacitySteel else 551)
    "Specific heat capacity of steel, in J/kgK" annotation (Dialog(
      tab="Geometry and Material",
      group="Material",
      enable=not selectable));

  parameter Modelica.Units.SI.ThermalConductivity lambdaSteel=(if selectable
       then radiatorType.LambdaSteel else 60)
    "Thermal conductivity of steel, in W/mK" annotation (Dialog(
      tab="Geometry and Material",
      group="Material",
      enable=not selectable));
  ...
  parameter Modelica.Units.SI.Temperature RT_nom[3]=(if selectable then
...
//     "Specific density of water, in kg/m3";
  ...
  Modelica.Units.SI.Temperature TV_1;
  Modelica.Units.SI.Temperature TR_N;
protected
  parameter Modelica.Units.SI.Temperature T0_N[N]={(T0 - ki*0.2) for ki in 1:N};
  parameter Modelica.Units.SI.Volume vol_water=(length*volumeWater/1000)/N;
  parameter Modelica.Units.SI.Volume vol_steel=(length*massSteel)/densitySteel/
      N annotation (Dialog(tab="Geometry and Material", group="Geometry"));

  parameter Modelica.Units.SI.Length d1=2*(vol_water/Modelica.Constants.pi/
      length)^0.5 "inner diameter of single layer";
  parameter Modelica.Units.SI.Length d2=2*((vol_water + vol_steel)/Modelica.Constants.pi
      /length)^0.5 "outer diameter of single layer";
  ...
  parameter Modelica.Units.SI.Temperature delta_nom=(dT_nom + RT_nom[3])*(
      dT_nom + RT_nom[3])*(dT_nom + RT_nom[3])*(dT_nom + RT_nom[3]) - RT_nom[3]
      *RT_nom[3]*RT_nom[3]*RT_nom[3] "Nominal radiation excess temperature";
  parameter Modelica.Units.SI.Power dotQ_nomLayer=length*nominalPower/N
    "Nominal heat flow";
  /* *******************************************************************
      Components
     ******************************************************************* */
